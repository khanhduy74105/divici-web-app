...
class productController {
    async addToCart(req, res){
        try {
            ...
            if (existProduct) {
                ...
                res.json({success: true, message:'Add to cart success', updatedCart})
            }
            else{
    ...
    async getDetail(req, res){
        try {
            ...
            const rate = comments.reduce((total, curr)=> total + curr.rate , 0) / comments.length
            const listRate ={
                '1sao': 0,
                '2sao': 0,
                '3sao': 0,
    ...
    async update(req, res) {
        try {
            ...
            const updatedCart = await Cart.findOneAndUpdate(cartCondition, newCart,{new: true})
            res.json({success: true, message: 'Update cart success', updatedCart: updatedCart})
    ...
    async getProductsType (req, res){
        try {
            const productPerPage = 12
            const data = await Product.find({type: req.params.type}).limit(productPerPage)
